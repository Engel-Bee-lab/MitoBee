...
rule fastp:
    input:
        r1 = lambda wc: os.path.join(config["input"], f"{wc.sample}{config['pattern_r1']}.{config['extn'][0]}"),
        r2 = lambda wc: os.path.join(config["input"], f"{wc.sample}{config['pattern_r2']}.{config['extn'][0]}")
    output:
