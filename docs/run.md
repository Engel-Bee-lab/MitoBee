# Reference-based Mitogenome Extraction

This module of **MitoBee** reconstructs a mitochondrial genome by extracting reads that map to a user-provided reference mitogenome. The goal is to recover a consensus sequence for each sample using a closely related mitochondrial reference.
...
## Input 
- Metagenomic reads, paired end 
- Reference host

## Example command 
There are two modes for running this command
1. mitogenome: Where the complete mtDNA will be used to build a consensus mtDNA from each metagenome sample
...
2. genes: In this case, the user can provide a closely related core mtgene. 
